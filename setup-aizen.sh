...
# 5. Symlinks the 'aizen' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Aizen Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink aizen into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up aizen command..."

AIZEN_BIN="$SCRIPT_DIR/venv/bin/aizen"
mkdir -p "$HOME/.local/bin"
ln -sf "$AIZEN_BIN" "$HOME/.local/bin/aizen"
echo -e "${GREEN}✓${NC} Symlinked aizen → ~/.local/bin/aizen"
...
if [ -n "$SHELL_CONFIG" ]; then
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
            echo "" >> "$SHELL_CONFIG"
            echo "# Aizen Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.aizen/skills/
# ============================================================================

AIZEN_SKILLS_DIR="${AIZEN_HOME:-$HOME/.aizen}/skills"
mkdir -p "$AIZEN_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.aizen/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$AIZEN_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  aizen status        # Check configuration"
echo "  aizen gateway install # Install gateway service (messaging + cron)"
echo "  aizen cron list     # View scheduled jobs"
echo "  aizen doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m aizen_cli.main setup
fi
