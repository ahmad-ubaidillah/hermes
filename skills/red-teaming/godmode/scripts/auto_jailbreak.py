...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.aizen/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AIZEN_HOME", Path.home() / ".aizen")) / "skills" / "red-teaming" / "godmode"
...
AIZEN_HOME = Path(os.getenv("AIZEN_HOME", Path.home() / ".aizen"))
CONFIG_PATH = AIZEN_HOME / "config.yaml"
PREFILL_PATH = AIZEN_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Aizen models — already uncensored, just needs clean prompt
    "aizen": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "aizen" in model_lower or "nous" in model_lower:
        return "aizen"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Aizen config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.aizen/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Aizen for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Aizen for changes to take effect.")
