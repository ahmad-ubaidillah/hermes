...
## 5. GODMODE FAST — Aizen 4 405B (Zero Refusal)

**Model:** `nousresearch/aizen-4-405b`
**Strategy:** Aizen models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Aizen Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.aizen/config.yaml`:
