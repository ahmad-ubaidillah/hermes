...
- Give it a name like "aizen-agent"
- Select scopes:
...
- Give it a title like "aizen-agent-<machine-name>"
...
elif [ -f ~/.aizen/.env ] && grep -q "^GITHUB_TOKEN=" ~/.aizen/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.aizen/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
