...
Checklist for integrating a new messaging platform into the Aizen gateway.
...
And add it to the `aizen-gateway` composite:

```python
"aizen-gateway": {
    "includes": [..., "aizen-your-platform"]
}
```
...
## 12. Status Display (`aizen_cli/status.py`)
...
## 13. Gateway Setup Wizard (`aizen_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ aizen_cli/ toolsets.py \
  --include="*.py" -l | sort -u
