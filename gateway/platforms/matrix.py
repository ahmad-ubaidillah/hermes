...
# Uses get_aizen_home() so each profile gets its own Matrix store.
from core.aizen_constants import get_aizen_dir as _get_aizen_dir
_STORE_DIR = _get_aizen_dir("platforms/matrix/store", "matrix/store")
...
_KEY_EXPORT_PASSPHRASE = "aizen-matrix-e2ee-keys"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _run_e2ee_maintenance(self) -> None:
        ...
        Aizen uses a custom sync loop instead of matrix-nio's sync_forever(),
