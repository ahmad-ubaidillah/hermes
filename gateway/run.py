...
# ---------------------------------------------------------------------------
# Input validation — protects the gateway from oversized / malformed messages
# ---------------------------------------------------------------------------

# Default max message length: 100 KB (configurable via security.max_message_length)
_DEFAULT_MAX_MESSAGE_LENGTH = 102_400
...
def _get_max_message_length(config: Optional[dict] = None) -> int:
    ...
    if config and isinstance(config, dict):
        ...
        if isinstance(security, dict):
            val = security.get("max_message_length")
            if val is not None:
                try:
                    limit = int(val)
...
def _sanitize_message(text: str, config: Optional[dict] = None) -> str:
    ...
    Preserves TAB, LF, CR which are normal whitespace in text.
    Only runs when security.sanitize_input is True (default).
    """
    if config and isinstance(config, dict):
        security = config.get("security", {}) or {}
        if isinstance(security, dict) and not security.get("sanitize_input", True):
...
def _validate_message_length(text: str, max_length: int) -> Optional[str]:
    ...
    if len(text.encode("utf-8")) > max_length:
        return (
            f"Message too large ({len(text.encode('utf-8')):,} bytes). "
...
def _validate_message(
    ...
    """Run all input validations on a MessageEvent.

    Returns an error message string if validation fails, or None if OK.
    """
    if event is None or not event.text:
        return None  # empty messages are handled elsewhere

    # 1. Length check
    max_len = _get_max_message_length(config)
    length_error = _validate_message_length(event.text, max_len)
    if length_error:
        return length_error

    # 2. Sanitize in-place (mutates event.text)
    event.text = _sanitize_message(event.text, config)
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # Rate limiter for per-user/chat message throttling
        from gateway.rate_limiter import RateLimiter

        self.rate_limiter = RateLimiter.from_dict(
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        # ── Input validation ────────────────────────────────────────────
        # Validate message length and sanitize control characters before
        # any other processing.  Returns an error string on failure.
        validation_error = _validate_message(event, _load_gateway_config())
        if validation_error:
            logger.warning(
