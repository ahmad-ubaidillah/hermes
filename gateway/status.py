...
The PID file lives at ``{AIZEN_HOME}/gateway.pid``.  AIZEN_HOME defaults to
``~/.aizen`` but can be overridden via the environment variable.  This means
separate AIZEN_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "aizen-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting AIZEN_HOME."""
    home = get_aizen_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("AIZEN_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "aizen" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Aizen gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different AIZEN_HOME dirs).
    """
