...
Uses a file-based lock (~/.aizen/cron/.tick.lock) so only one tick
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_aizen_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if origin:
        os.environ["AIZEN_SESSION_PLATFORM"] = origin["platform"]
        os.environ["AIZEN_SESSION_CHAT_ID"] = str(origin["chat_id"])
        if origin.get("chat_name"):
            os.environ["AIZEN_SESSION_CHAT_NAME"] = origin["chat_name"]
    ...
    try:
        ...
        try:
            load_dotenv(str(_aizen_home / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_aizen_home / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            os.environ["AIZEN_CRON_AUTO_DELIVER_PLATFORM"] = delivery_target["platform"]
            os.environ["AIZEN_CRON_AUTO_DELIVER_CHAT_ID"] = str(delivery_target["chat_id"])
            if delivery_target.get("thread_id") is not None:
                os.environ["AIZEN_CRON_AUTO_DELIVER_THREAD_ID"] = str(delivery_target["thread_id"])

        model = job.get("model") or os.getenv("AIZEN_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_aizen_home / "config.yaml")
        ...
        prefill_file = os.getenv("AIZEN_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            import json as _json
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _aizen_home / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            runtime_kwargs = {
                "requested": job.get("provider") or os.getenv("AIZEN_INFERENCE_PROVIDER"),
            }
            if job.get("base_url"):
...
**Job ID:** {job_id}
**Run Time:** {_aizen_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        for key in (
            ...
            "AIZEN_CRON_AUTO_DELIVER_PLATFORM",
            "AIZEN_CRON_AUTO_DELIVER_CHAT_ID",
            "AIZEN_CRON_AUTO_DELIVER_THREAD_ID",
        ):
...
def tick(verbose: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _aizen_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _aizen_now().strftime('%H:%M:%S'), len(due_jobs))
