...
_loaded_env_paths = load_aizen_dotenv(aizen_home=_aizen_home, project_env=_project_env)
...
# =============================================================================
# SECTION 3: AIAgent Class Definition
# =============================================================================
...
class AIAgent:
    ...
    def __init__(
        ...
        model: str = "",  # Empty — user must configure via setup
    ...
    # =============================================================================
    # SECTION 4: Core Conversation Loop - run_conversation method
    # =============================================================================
    ...
    def run_conversation(
        ...
        while (
            ...
            # If the provider/model circuit breaker is OPEN, skip the API call
            # and trigger fallback immediately instead of burning retries.
            _cb = get_provider_breaker(self.provider, self.model)
            if _cb.state == CircuitState.OPEN:
                self._vprint(
                    f"{self.log_prefix}⚡ Circuit breaker OPEN for {self.provider}/{self.model} — "
                    f"skipping API call, trying fallback...",
                    force=True,
                )
                if self._fallback_index < len(self._fallback_chain):
                    if self._try_activate_fallback():
                        # Retry with fallback provider on next outer-loop iteration
                ...
                return {
                    ...
                    "error": f"Circuit breaker OPEN for {self.provider}/{self.model}. "
                    f"Provider is experiencing persistent failures. No fallback available.",
                    "failed": True,
                }
            ...
            while retry_count < max_retries:
                try:
                    ...
                    # Record failure on circuit breaker (skip rate limits — transient, already handled)
                    if not is_rate_limited:
                        record_provider_failure(self.provider, self.model)
...
# =============================================================================
# SECTION 5: Entry Points - chat() and main()
# =============================================================================
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n\n❌ Fatal error: {e}")
        print("📋 Stack trace logged to ~/.aizen/logs/errors.log")
        sys.exit(1)
