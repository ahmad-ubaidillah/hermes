# Contributing to Aizen Agent

Thank you for contributing to Aizen Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Aizen to work everywhere.
...
Bundled skills (in `skills/`) ship with every Aizen install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `aizen skills browse` (labeled "official") and install it with `aizen skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `aizen skills install`.
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/aizen-agent.git
cd aizen-agent
...
```bash
mkdir -p ~/.aizen/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.aizen/config.yaml
touch ~/.aizen/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.aizen/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/aizen" ~/.local/bin/aizen
...
├── cli.py                    # AizenCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (aizen-cli, aizen-telegram, etc.)
├── aizen_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── aizen_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.aizen/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (aizen-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.aizen/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.aizen/`)

| Path | Purpose |
|------|---------|
| `~/.aizen/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.aizen/.env` | API keys and secrets |
| `~/.aizen/auth.json` | OAuth credentials (Nous Portal) |
| `~/.aizen/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.aizen/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.aizen/state.db` | SQLite session database |
| `~/.aizen/sessions/` | JSON session logs |
| `~/.aizen/cron/` | Scheduled job data |
| `~/.aizen/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`aizen_state.py`) with full-text search and unique session titles. JSON logs go to `~/.aizen/sessions/`.
...
Four fields are supported under `metadata.aizen`:
...
The user may skip setup and keep loading the skill. Aizen only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `aizen setup` or update `~/.aizen/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Aizen tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `aizen --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Aizen uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.aizen/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `aizen_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `aizen_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Aizen runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Aizen has terminal access. Security matters.
...
2. **Test manually**: Run `aizen` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/aizen-agent/issues)
- Include: OS, Python version, Aizen version (`aizen version`), full error traceback
