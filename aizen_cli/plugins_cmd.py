"""``aizen plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.aizen/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    aizen_home = os.environ.get("AIZEN_HOME", os.path.expanduser("~/.aizen"))
    plugins = Path(aizen_home) / "plugins"
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]aizen plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Aizen plugin."
        )
    ...
    console.print("[dim]  aizen gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] aizen plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via aizen plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] aizen plugins")
    console.print("[dim]Enable/disable:[/dim] aizen plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch aizen plugins subcommands."""
