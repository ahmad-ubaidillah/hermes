#!/usr/bin/env python3
"""
Aizen CLI - Main entry point.

Usage:
    aizen                     # Interactive chat (default)
    aizen chat                # Interactive chat
    aizen gateway             # Run gateway in foreground
    aizen gateway start       # Start gateway as service
    aizen gateway stop        # Stop gateway service
    aizen gateway status      # Show gateway status
    aizen gateway install     # Install gateway service
    aizen gateway uninstall   # Uninstall gateway service
    aizen setup               # Interactive setup wizard
    aizen logout              # Clear stored authentication
    aizen status              # Show status of all components
    aizen cron                # Manage cron jobs
    aizen cron list           # List cron jobs
    aizen cron status         # Check if cron scheduler is running
    aizen doctor              # Check configuration and dependencies
    aizen honcho setup                    # Configure Honcho AI memory integration
    aizen honcho status                   # Show Honcho config and connection status
    aizen honcho sessions                 # List directory → session name mappings
    aizen honcho map <name>               # Map current directory to a session name
    aizen honcho peer                     # Show peer names and dialectic settings
    aizen honcho peer --user NAME         # Set user peer name
    aizen honcho peer --ai NAME           # Set AI peer name
    aizen honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    aizen honcho mode                     # Show current memory mode
    aizen honcho mode [hybrid|honcho|local]  # Set memory mode
    aizen honcho tokens                   # Show token budget settings
    aizen honcho tokens --context N       # Set session.context() token cap
    aizen honcho tokens --dialectic N     # Set dialectic result char cap
    aizen honcho identity                 # Show AI peer identity representation
    aizen honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    aizen honcho migrate                  # Step-by-step migration guide: OpenClaw native → Aizen + Honcho
    aizen version             Show version
    aizen update              Update to latest version
    aizen uninstall           Uninstall Aizen Agent
    aizen acp                 Run as an ACP server for editor integration
    aizen sessions browse     Interactive session picker with search

    aizen claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (aizen tools, aizen setup, aizen model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'aizen {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any aizen module import.
#
# Many modules cache AIZEN_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("AIZEN_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.aizen/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set AIZEN_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.aizen/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".aizen" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set AIZEN_HOME
    if profile_name is not None:
        try:
            from aizen_cli.profiles import resolve_profile_env
            aizen_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent aizen from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["AIZEN_HOME"] = aizen_home
...
load_aizen_dotenv(project_env=PROJECT_ROOT / '.env')
...
from aizen_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from aizen_cli.config import get_env_path, get_aizen_home
    from aizen_cli.auth import get_auth_status
    ...
    auth_file = get_aizen_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'aizen sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Aizen isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  aizen setup")
        print()

        from aizen_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'aizen setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from aizen_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["AIZEN_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from aizen_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from aizen_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Aizen?")
        print()
    ...
    session_dir = get_aizen_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: aizen gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  aizen gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Aizen Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  aizen gateway")
        ...
        print()
        print("  Or install as a service: aizen gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'aizen whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from aizen_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    _require_tty("model")
    from aizen_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from aizen_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("AIZEN_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from aizen_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from aizen_cli.config import get_env_value, save_env_value
    ...
    from aizen_cli.models import model_ids
    ...
    if selected:
        ...
        from aizen_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model=""):
    ...
    from aizen_cli.models import _PROVIDER_MODELS
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from aizen_cli.codex_models import get_codex_model_ids
    from aizen_cli.config import get_env_value, save_env_value
    import argparse
...
def _model_flow_custom(config):
    ...
    """
    from aizen_cli.auth import _save_model_choice, deactivate_provider
    from aizen_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from aizen_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `aizen model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from aizen_cli.auth import _save_model_choice, deactivate_provider
    from aizen_cli.config import save_env_value, load_config, save_config
    from aizen_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from aizen_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from aizen_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Aizen turns to `copilot --acp`.")
    print("  Aizen currently starts its own ACP subprocess for each request.")
    print("  Aizen uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set AIZEN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(f"    Aizen will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'aizen model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Aizen CLI with a provider."""
    from aizen_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from aizen_cli.auth import logout_command
    logout_command(args)


def cmd_status(args):
    """Show status of all components."""
    from aizen_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from aizen_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from aizen_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from aizen_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from aizen_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Aizen Agent v{__version__} ({__release_date__})")
    ...
    try:
        from aizen_cli.banner import check_for_updates
        from aizen_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Aizen Agent."""
    _require_tty("uninstall")
    from aizen_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update Aizen Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/aizen-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="aizen-update-")
        zip_path = os.path.join(tmp_dir, f"aizen-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to aizen-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"aizen-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("aizen-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Aizen behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Aizen can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Aizen couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Aizen couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Aizen behaves unexpectedly.")
    return True

def _invalidate_update_cache():
    """Delete the update-check cache so ``aizen --version`` doesn't
    report a stale "commits behind" count after a successful update."""
    try:
        cache_file = Path(os.getenv(
            "AIZEN_HOME", Path.home() / ".aizen"
        )) / ".update_check"
...
def cmd_update(args):
    """Update Aizen Agent to the latest version."""
    import shutil
    from aizen_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Aizen Agent")
        return
    
    print("⚕ Updating Aizen Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/aizen-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_aizen_home added to aizen_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload aizen_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_aizen_home() added since the last release.
        try:
            import importlib
            import core.aizen_constants as _hc
        ...
        try:
            from aizen_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'aizen config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'aizen config migrate' later to configure.")
        else:
        ...
        # Uses the PID file (scoped to AIZEN_HOME) to find this
        # installation's gateway — safe with multiple installations.
        try:
            from gateway.status import get_running_pid, remove_pid_file
            from aizen_cli.gateway import (
            ...
            # Also check for a system-level service (aizen gateway install --system).
            ...
            if is_macos():
                try:
                    from aizen_cli.gateway import get_launchd_label
            ...
            if existing_pid or has_systemd_service or has_system_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    aizen gateway restart")
                            else:
                                print("  Try manually: aizen gateway restart")
                elif has_system_service:
                    # System-level service (aizen gateway install --system).
                    ...
                    print("  Restart it with: aizen gateway run")
        except Exception as e:
            logger.debug("Gateway restart during update failed: %s", e)
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  aizen model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``aizen -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from aizen_cli.profiles import (
    ...
    from core.aizen_constants import display_aizen_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `aizen profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_aizen_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → aizen -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.aizen)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  aizen profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     aizen -p {name} chat")
                else:
            ...
            if clone or clone_all:
                ...
                profile_dir_display = f"~/.aizen/profiles/{name}"
        ...
        name = args.profile_name
        from aizen_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from aizen_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from aizen_cli.profiles import rename_profile
        try:
        ...
        from aizen_cli.profiles import export_profile
        name = args.profile_name
        ...
        from aizen_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from aizen_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for aizen CLI."""
    parser = argparse.ArgumentParser(
        prog="aizen",
        description="Aizen Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    aizen                        Start interactive chat
    aizen chat -q "Hello"        Single query mode
    aizen -c                     Resume the most recent session
    aizen -c "my project"        Resume a session by name (latest in lineage)
    aizen --resume <session_id>  Resume a specific session by ID
    aizen setup                  Run setup wizard
    aizen logout                 Clear stored authentication
    aizen model                  Select default model
    aizen config                 View configuration
    aizen config edit            Edit config in $EDITOR
    aizen config set model gpt-4 Set a config value
    aizen gateway                Run messaging gateway
    aizen -s aizen-agent-dev,github-auth
    aizen -w                     Start in isolated git worktree
    aizen gateway install        Install gateway background service
    aizen sessions list          List past sessions
    aizen sessions browse        Interactive session picker
    aizen sessions rename ID T   Rename/title a session
    aizen update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Aizen Agent"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Aizen Agent with an interactive wizard. "
                    "Run a specific section: aizen setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Aizen CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: aizen-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Aizen Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Aizen Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Aizen Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from aizen_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from aizen_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from aizen_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/aizen-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from aizen_cli.plugins_cmd import plugins_command
    ...
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "each turn so Aizen knows who you are without you having to repeat yourself.\n\n"
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Aizen Honcho",
    )
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'aizen tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # aizen tools disable <name...> [--platform cli]
    ...
    # aizen tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from aizen_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from aizen_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Aizen as an MCP server",
        description=(
            "Manage MCP server connections and run Aizen as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'aizen mcp add' to connect to a new server, or\n"
            "'aizen mcp serve' to expose Aizen conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Aizen as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from aizen_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch aizen --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            aizen_bin = shutil.which("aizen")
            if aizen_bin:
                os.execvp(aizen_bin, ["aizen", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "aizen_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Aizen"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Aizen",
    ...
    def cmd_claw(args):
        from aizen_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Aizen Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Aizen Agent",
        description="Remove Aizen Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Aizen Agent as an ACP (Agent Client Protocol) server",
        description="Start Aizen Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Aizen Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Aizen instances",
    )
    ...
    # e.g. ``aizen -c Pokemon Agent Dev`` → ``aizen -c 'Pokemon Agent Dev'``
