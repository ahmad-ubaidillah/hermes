"""aizen webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    aizen webhook subscribe <name> [options]
    aizen webhook list
    aizen webhook remove <name>
    aizen webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.aizen/webhook_subscriptions.json and are
...
def _aizen_home() -> Path:
    return Path(
        os.getenv("AIZEN_HOME", str(Path.home() / ".aizen"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _aizen_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: aizen gateway run
"""
...
def webhook_command(args):
    """Entry point for 'aizen webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: aizen webhook {subscribe|list|remove|test}")
        print("Run 'aizen webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (aizen gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: aizen webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from aizen webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (aizen gateway run)")
