"""
Interactive setup wizard for Aizen Agent.
...
Config files are stored in ~/.aizen/ for easy access.
"""
...
from core.aizen_constants import get_optional_skills_dir
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from aizen_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from aizen_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from aizen_cli.config import get_env_value
...
# display_aizen_home imported lazily at call sites (stale-module safety during aizen update)

from aizen_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Aizen Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Aizen using environment variables or config commands:")
    print_info("  aizen config set model.provider custom")
    print_info("  aizen config set model.base_url http://localhost:8080/v1")
    print_info("  aizen config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'aizen setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from aizen_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'aizen setup')")


def _print_setup_summary(config: dict, aizen_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'aizen setup' to configure"))
    ...
    if tts_provider == "elevenlabs" and get_env_value("ELEVENLABS_API_KEY"):
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'aizen setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'aizen setup tools' to configure them,"
        )
        from core.aizen_constants import display_aizen_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {aizen_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('aizen setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('aizen setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('aizen setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('aizen setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('aizen setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('aizen config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('aizen config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('aizen config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('aizen', Colors.GREEN)}              Start chatting")
    print(f"   {color('aizen gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('aizen doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `aizen tools` and `aizen setup tools`).
...
def setup_model_provider(config: dict):
    ...
    if not has_any_provider:
        print_warning("An inference provider is required for Aizen to work.")
        print()
    ...
    if provider_idx == 0:  # OpenRouter
        ...
        try:
            from aizen_cli.auth import deactivate_provider
        ...
        config_path = (
            Path(os.environ.get("AIZEN_HOME", Path.home() / ".aizen")) / "config.yaml"
        )
        try:
            disk_cfg = {}
        ...
        try:
            from aizen_cli.auth import _login_nous
            import argparse
            ...
            print_info("You can try again later with: aizen model")
        ...
        # Reuse the shared custom endpoint flow from `aizen model`.
        ...
        from aizen_cli.main import _model_flow_custom
        ...
        if api_key:
            ...
            from aizen_cli.auth import detect_zai_endpoint
        ...
        from aizen_cli.config import save_anthropic_api_key, save_anthropic_oauth_token
        ...
        print_info("Aizen can use GITHUB_TOKEN, GH_TOKEN, or your gh CLI login.")
        print_info(f"Base URL: {pconfig.inference_base_url}")
        print()
        ...
        print_info("Aizen will start `copilot --acp --stdio` for each request.")
        print_info("Use AIZEN_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH to override the command.")
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Aizen can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Aizen can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'aizen setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider != "custom":  # Custom already prompted for model name
        ...
        if selected_provider == "nous" and nous_models:
            ...
            from aizen_cli.models import provider_model_ids
            ...
            from aizen_cli.models import model_ids, menu_labels
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'aizen setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Aizen runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Aizen via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("AIZEN_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("AIZEN_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Aizen from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Aizen delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://aizen-agent.nousresearch.com/docs/user-guide/messaging/slack/"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where Aizen delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where Aizen delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'aizen whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'aizen whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://aizen-agent.nousresearch.com/docs/user-guide/messaging/webhooks/"
        )
        print()
        ...
        print_info(
            "   https://aizen-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  aizen config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     aizen config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: aizen gateway install")
            else:
                print_info("  You can install later: aizen gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo aizen gateway install --system")
                print_info("  Or run in foreground:  aizen gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   aizen gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `aizen setup tools` and `aizen tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from aizen_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from aizen_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_aizen.py"
)


def _offer_openclaw_migration(aizen_home: Path) -> bool:
    ...
    print_info("Aizen can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_aizen", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=aizen_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Aizen.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      aizen setup           — full or quick (auto-detected)
      aizen setup model     — just model/provider
      aizen setup terminal  — just terminal backend
      aizen setup gateway   — just messaging platforms
      aizen setup tools     — just tool configuration
      aizen setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Aizen Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Aizen Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Aizen Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Aizen configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, aizen_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'aizen setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'aizen setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, aizen_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(aizen_home)
        if migration_ran:
    ...
    print_info(f"Data folder:  {aizen_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'aizen config edit'")
...
def _run_quick_setup(config: dict, aizen_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'aizen setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Aizen to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'aizen setup gateway'.")
