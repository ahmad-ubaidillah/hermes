"""Slash command definitions and autocomplete for the Aizen CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef(
        "opencode",
        "Collaborate with OpenCode: code, review, generate, explain",
        "Tools & Skills",
        args_hint="[task|review|generate|explain|status]",
        subcommands=("task", "review", "generate", "explain", "status", "edit"),
    ),
    CommandDef(
        "browser",
    ...
    CommandDef(
        "update", "Update Aizen Agent to the latest version", "Info", gateway_only=True
    ),
    # Exit
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("AIZEN_HOME", os.path.expanduser("~/.aizen")),
            "config.yaml",
        )
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /aizen handler.

    Maps both canonical names and aliases so /aizen bg do stuff works
    the same as /aizen background do stuff.
    """
    overrides = _resolve_config_gates()
