"""
Profile management for multiple isolated Aizen instances.

Each profile is a fully independent AIZEN_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.aizen/profiles/<name>/`` by default.

The "default" profile is ``~/.aizen`` itself — backward compatible,
zero migration needed.

Usage::

    aizen profile create coder          # fresh profile + bundled skills
    aizen profile create coder --clone  # also copy config, .env, SOUL.md
    aizen profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    aizen -p coder chat                 # or via flag
    aizen profile use coder             # set as sticky default
    aizen profile delete coder          # remove profile + alias + service
"""
...
_RESERVED_NAMES = frozenset({
    "aizen", "default", "test", "tmp", "root", "sudo",
})

# Aizen subcommands that cannot be used as profile names/aliases
_AIZEN_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.aizen/profiles/`` — anchored to the user's home,
    NOT to the current AIZEN_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".aizen" / "profiles"


def _get_default_aizen_home() -> Path:
    """Return the default (pre-profile) AIZEN_HOME path."""
    return Path.home() / ".aizen"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_aizen_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.aizen
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its AIZEN_HOME directory."""
    if name == "default":
        return _get_default_aizen_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, aizen subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _AIZEN_SUBCOMMANDS:
        return f"'{name}' conflicts with a aizen subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_aizen_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.aizen)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches AIZEN_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AIZEN_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.aizen).\n"
            "To remove everything, use: aizen uninstall"
        )
    ...
    # Check for service
    from aizen_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set AIZEN_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("AIZEN_HOME")
    try:
        os.environ["AIZEN_HOME"] = str(profile_dir)
        from aizen_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["AIZEN_HOME"] = old_home
        elif "AIZEN_HOME" in os.environ:
            del os.environ["AIZEN_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.aizen/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: aizen profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from AIZEN_HOME.

    Returns ``"default"`` if AIZEN_HOME is not set or points to ``~/.aizen``.
    Returns the profile name if AIZEN_HOME points into ``~/.aizen/profiles/<name>``.
    Returns ``"custom"`` if AIZEN_HOME is set to an unrecognized path.
    """
    from core.aizen_constants import get_aizen_home
    aizen_home = get_aizen_home()
    resolved = aizen_home.resolve()

    default_resolved = _get_default_aizen_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: aizen profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for aizen profile names."""
    return '''# Aizen Agent profile completion
# Add to ~/.bashrc: eval "$(aizen completion bash)"

_aizen_profiles() {
    local profiles_dir="$HOME/.aizen/profiles"
...
_aizen_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_aizen_profiles)" -- "$cur"))
        return
    fi
...
complete -F _aizen_completion aizen
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for aizen profile names."""
    return '''#compdef aizen
# Aizen Agent profile completion
# Add to ~/.zshrc: eval "$(aizen completion zsh)"

_aizen() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.aizen/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.aizen/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a AIZEN_HOME path string.

    Called early in the CLI entry point, before any aizen modules
    are imported, to set the AIZEN_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: aizen profile create {profile_name}"
        )
