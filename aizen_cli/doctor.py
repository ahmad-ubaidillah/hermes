"""
Doctor command for aizen CLI.

Diagnoses issues with Aizen Agent setup.
"""
...
from aizen_cli.config import get_project_root, get_aizen_home, get_env_path
from core.aizen_constants import display_aizen_home

PROJECT_ROOT = get_project_root()
AIZEN_HOME = get_aizen_home()
_DHH = display_aizen_home()  # user-facing display path (e.g. ~/.aizen or ~/.aizen/profiles/coder)

# Load environment variables from ~/.aizen/.env so API key checks work
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.aizen/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `aizen`.
    os.environ.setdefault("AIZEN_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Aizen Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.aizen/.env (primary location for user config)
    env_path = AIZEN_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'aizen setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'aizen setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'aizen setup' to create one")
                issues.append("Run 'aizen setup' to create .env")
    
    # Check ~/.aizen/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    try:
        from aizen_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if aizen_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            aizen_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = aizen_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Aizen a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Aizen Agent Persona\n\n"
                "<!-- Edit this file to customize how Aizen communicates. -->\n\n"
                "You are Aizen, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = aizen_home / "memories"
    ...
    state_db_path = aizen_home / "state.db"
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/aizen-agent/issues/811
    ...
    try:
        ...
        from tools.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
        ...
        if api_disabled:
            issues.append("Run 'aizen setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: aizen skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: aizen honcho setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: aizen honcho setup")
            issues.append("No Honcho API key — run 'aizen honcho setup'")
        else:
    ...
    # =========================================================================
    try:
        from aizen_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "aizen -p" in content:
                            _m = _re.search(r"aizen -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'aizen doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
