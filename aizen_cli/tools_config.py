"""
Unified tool configuration for Aizen Agent.

`aizen tools` and `aizen setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.aizen/config.yaml under
...
# They're still in _AIZEN_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from aizen_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "aizen-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "aizen-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "aizen-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "aizen-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "aizen-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "aizen-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "aizen-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "aizen-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "aizen-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "aizen-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "aizen-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "aizen-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "aizen-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "aizen-mattermost"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_aizen_home()}/aizen-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in aizen-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "aizen-cli" (which include all _AIZEN_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "aizen-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `aizen tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by aizen tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (aizen-cli, aizen-telegram, etc.)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `aizen tools` and `aizen setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Aizen Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from core.aizen_constants import display_aizen_home
    print(color(f"  Tool configuration saved to {display_aizen_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'aizen' or gateway restart.", Colors.DIM))
    print()
