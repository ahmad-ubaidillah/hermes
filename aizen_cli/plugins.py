...
1. **User plugins**   – ``~/.aizen/plugins/<name>/``
2. **Project plugins** – ``./.aizen/plugins/<name>/`` (opt-in via
   ``AIZEN_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``aizen_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "aizen_agent.plugins"

_NS_PARENT = "aizen_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.aizen/plugins/)
        aizen_home = os.environ.get("AIZEN_HOME", os.path.expanduser("~/.aizen"))
        user_dir = Path(aizen_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.aizen/plugins/)
        if _env_enabled("AIZEN_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".aizen" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``aizen_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``aizen tools`` TUI so plugin-provided toolsets appear
