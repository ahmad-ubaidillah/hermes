"""
Gateway subcommand for aizen CLI.

Handles: aizen gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from aizen_cli.config import get_env_value, get_aizen_home, save_env_value, is_managed, managed_error
# display_aizen_home is imported lazily at call sites to avoid ImportError
# when aizen_constants is cached from a pre-update version during `aizen update`.
from aizen_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "aizen_cli.main gateway",
        "aizen_cli/main.py gateway",
        "aizen gateway",
        "gateway/run.py",
    ]
...
_SERVICE_BASE = "aizen-gateway"
SERVICE_DESCRIPTION = "Aizen Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current AIZEN_HOME.

    Returns ``""`` for the default ``~/.aizen``, the profile name for
    ``~/.aizen/profiles/<name>``, or a short hash for any other custom
    AIZEN_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_aizen_home().resolve()
    default = (_Path.home() / ".aizen").resolve()
    if home == default:
        return ""
    # Detect ~/.aizen/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary AIZEN_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this AIZEN_HOME.

    Default ``~/.aizen`` returns ``aizen-gateway`` (backward compatible).
    Profile ``~/.aizen/profiles/coder`` returns ``aizen-gateway-coder``.
    Any other AIZEN_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    aizen gateway uninstall")
    print_info("    sudo aizen gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Aizen can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo aizen gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo aizen gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo aizen gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.aizen`` → ``ai.aizen.gateway.plist`` (backward compatible).
    Profile ``~/.aizen/profiles/coder`` → ``ai.aizen.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.aizen.gateway-{suffix}" if suffix else "ai.aizen.gateway"
...
def get_aizen_cli_path() -> str:
    """Get the path to the aizen CLI."""
    ...
    return f"{get_python_path()} -m aizen_cli.main"
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    aizen_home = str(get_aizen_home().resolve())
...
User={username}
Group={group_name}
ExecStart={python_path} -m aizen_cli.main gateway run --replace
...
Environment="AIZEN_HOME={aizen_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Aizen install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}aizen gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}aizen gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}aizen gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}aizen gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}aizen gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.aizen.gateway-{suffix}" if suffix else "ai.aizen.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Aizen install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  aizen gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Aizen install")
    else:
        print("⚠ Service definition is stale relative to the current Aizen install")
        print("  Run: aizen gateway start")
    ...
    if deep:
        log_file = get_aizen_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Aizen will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @aizen:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. aizen) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Aizen delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Aizen agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Aizen will use (e.g., aizen@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_aizen_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'aizen pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: aizen pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'aizen gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from aizen_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"AizenAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: aizen gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: aizen gateway")
    else:
        print()
        print_info("No platforms configured. Run 'aizen gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: aizen gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: aizen gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  aizen gateway install")
                print("  sudo aizen gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  aizen gateway          # Run in foreground")
                print("  aizen gateway install  # Install as user service")
                print("  sudo aizen gateway install --system  # Install as boot-time system service")
