...
from aizen_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from aizen_cli.models import copilot_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("AIZEN_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    *,
    ...
    if provider == "nous":
        creds = resolve_nous_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("AIZEN_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
    ...
    if provider == "openai-codex":
        ...
        return {
            ...
            "base_url": creds.get("base_url", "").rstrip("/"),
            "api_key": creds.get("api_key", ""),
            "source": creds.get("source", "aizen-auth-store"),
