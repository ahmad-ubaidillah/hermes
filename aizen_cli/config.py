"""
Configuration management for Aizen Agent.

Config files are stored in ~/.aizen/ for easy access:
- ~/.aizen/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.aizen/.env         - API keys and secrets

This module provides:
- aizen config          - Show current configuration
- aizen config edit     - Open config in editor
- aizen config set      - Set a specific value
- aizen config wizard   - Re-run setup wizard
"""
...
from aizen_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("AIZEN_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_aizen_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Aizen is running in package-manager-managed mode.

    Two signals: the AIZEN_MANAGED env var (set by the systemd service),
    or a .managed marker file in AIZEN_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade aizen-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "aizen update"


def format_managed_message(action: str = "modify this Aizen installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("AIZEN_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Aizen installation is managed by NixOS "
            f"(AIZEN_MANAGED={env_hint}).\n"
            "Edit services.aizen-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Aizen installation is managed by Homebrew "
            f"(AIZEN_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade aizen-agent"
        )

    return (
        f"Cannot {action}: this Aizen installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Aizen."
    )
...
# Re-export from aizen_constants — canonical definition lives there.
from core.aizen_constants import get_aizen_home  # noqa: F811,E402


def get_config_path() -> Path:
    """Get the main config file path."""
    return get_aizen_home() / "config.yaml"


def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_aizen_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into AIZEN_HOME if the user doesn't have one yet."""
...
def ensure_aizen_home():
    """Ensure ~/.aizen directory structure exists with secure permissions."""
    home = get_aizen_home()
...
DEFAULT_CONFIG = {
    ...
    "code_execution": {
        ...
        "docker_network": "none",
        "docker_disk_read_bps": "10mb",
        "docker_disk_write_bps": "10mb",
    ...
    # always goes to ~/.aizen/skills/.
    "skills": {
        "external_dirs": [],  # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },
    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for aizen-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Aizen Agent*" header.
    ...
    "security": {
        ...
        "max_message_length": 102400,  # 100 KB — max gateway message size (0 = unlimited)
        "sanitize_input": True,  # Strip control characters and null bytes from incoming messages
        "website_blocklist": {
            "enabled": False,
            "domains": [],
            "shared_files": [],
...
OPTIONAL_ENV_VARS = {
    ...
    "WANDB_API_KEY": {
        "description": "(deprecated) Weights & Biases API key for experiment tracking — moved to optional; no longer in core toolset",
        "prompt": "WandB API key (deprecated)",
    ...
    # ── Honcho ──
    "HONCHO_API_KEY": {
        "description": "(deprecated) Honcho API key for AI-native persistent memory — honcho integration file does not exist; use local memory_tool.py instead",
        "prompt": "Honcho API key (deprecated — use local memory_tool.py)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @aizen:example.org)",
    ...
    # AIZEN_TOOL_PROGRESS and AIZEN_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("AIZEN_TOOL_PROGRESS")
            old_mode = get_env_value("AIZEN_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append(
                    "display.tool_progress=off (from AIZEN_TOOL_PROGRESS=false)"
                )
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(
                    f"display.tool_progress={old_mode.lower()} (from AIZEN_TOOL_PROGRESS_MODE)"
                )
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("AIZEN_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(
                    f"timezone={old_tz.strip()} (from AIZEN_TIMEZONE)"
                )
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: aizen config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.aizen/config.yaml."""
    import copy
...
#   openai-codex (OAuth — aizen login) — OpenAI Codex
#   nous         (OAuth — aizen login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.aizen/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.aizen/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Aizen env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.aizen/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.aizen/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.aizen/.env or environment."""
...
def show_config():
    ...
    print(
        color("│              ⚕ Aizen Configuration                    │", Colors.CYAN)
    )
    print(
        color(
            "└─────────────────────────────────────────────────────────┘", Colors.CYAN
    ...
    anthropic_value = get_env_value("ANTHROPIC_TOKEN") or get_env_value(
        "ANTHROPIC_API_KEY"
    )
    ...
    print(color("  aizen config edit     # Edit config file", Colors.DIM))
    print(color("  aizen config set <key> <value>", Colors.DIM))
    print(color("  aizen setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or not value:
            print("Usage: aizen config set <key> <value>")
            print()
            print("Examples:")
            print("  aizen config set model anthropic/claude-sonnet-4")
            print("  aizen config set terminal.backend docker")
            print("  aizen config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'aizen config migrate' to add them")
        ...
        print("  aizen config           Show current configuration")
        print("  aizen config edit      Open config in editor")
        print("  aizen config set <key> <value>   Set a config value")
        print("  aizen config check     Check for missing/outdated config")
        print("  aizen config migrate   Update config with new options")
        print("  aizen config path      Show config file path")
        print("  aizen config env-path  Show .env file path")
        sys.exit(1)
