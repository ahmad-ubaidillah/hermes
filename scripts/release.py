...
VERSION_FILE = REPO_ROOT / "aizen_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "aizen@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/aizen-agent",
    ...
    lines.append(f"# Aizen Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Aizen Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Aizen Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Aizen Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Aizen Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Aizen Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Aizen Agent v{new_version} ({calver_date})' "
