"""
AizenAgentBaseEnv -- Abstract Base Environment for Aizen-Agent + Atropos

Provides the Atropos integration plumbing that all aizen-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via AizenAgentLoop
...
# Ensure the aizen-agent repo root is on sys.path so that imports like
...
# Load API keys from aizen-agent/.env so all environments can access them
...
# Import aizen-agent toolset infrastructure
...
class AizenAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for aizen-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of aizen toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: aizen, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class AizenAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for aizen-agent Atropos integration.
    ...
    name: Optional[str] = "aizen-agent"
    env_config_cls = AizenAgentEnvConfig

    def __init__(
        self,
        config: AizenAgentEnvConfig,
        ...
        # Set terminal environment variables so aizen tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY aizen-agent tool (terminal, file, web,
