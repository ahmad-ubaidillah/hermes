...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs AizenAgentLoop with terminal tool only
...
Requires: pip install aizen-agent[yc-bench]
"""
...
class YCBenchEvalConfig(AizenAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends AizenAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(AizenAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The AizenAgentLoop then drives the
    ...
    async def setup(self):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                '  pip install "aizen-agent[yc-bench]"\n'
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs AizenAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our AizenAgentLoop.
