...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Aizen Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Aizen Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Aizen Agent",
    },
    {
...
class TerminalTestEnv(AizenAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/aizen-agent/.env.
        """
