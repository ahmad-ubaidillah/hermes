"""
AizenSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser aizen
    run-api
    python environments/aizen_swe_env.py serve \\
...
class AizenSweEnv(AizenAgentBaseEnv):
    ...
    name = "aizen-swe"
    env_config_cls = AizenSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[AizenSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = AizenSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepAizen-3-Llama-3-3B-Preview",
            tool_call_parser="aizen",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="aizen-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepAizen-3-Llama-3-3B-Preview",
