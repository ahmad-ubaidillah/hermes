...
from hermes_logging import get_logger, get_hermes_home
...
def check_disk_space(
    ...
    check_path = path or str(get_hermes_home())
    try:
...
def check_hermes_home() -> HealthCheckResult:
    """Check that HERMES_HOME directory exists and is writable."""
    home = get_hermes_home()
    if not home.exists():
        return HealthCheckResult(
            name="hermes_home",
            status=HealthStatus.UNHEALTHY,
            message=f"HERMES_HOME does not exist: {home}",
        )
    if not os.access(home, os.W_OK):
        return HealthCheckResult(
            name="hermes_home",
            status=HealthStatus.UNHEALTHY,
            message=f"HERMES_HOME is not writable: {home}",
        )
    return HealthCheckResult(
        name="hermes_home",
        status=HealthStatus.OK,
        message=f"HERMES_HOME OK: {home}",
...
def check_session_db(session_db_path: Optional[str] = None) -> HealthCheckResult:
    ...
    try:
        ...
        db_path = session_db_path or str(get_hermes_home() / "sessions.db")
...
def check_config_validity(config: Optional[dict] = None) -> HealthCheckResult:
    ...
    try:
        from hermes_config import validate_config

        if config is None:
            from hermes_config import load_config
