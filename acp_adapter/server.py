"""ACP agent server — exposes Aizen Agent via the Agent Client Protocol."""
...
try:
    from aizen_cli import __version__ as AIZEN_VERSION
except Exception:
    AIZEN_VERSION = "0.0.0"
...
class AizenACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Aizen AIAgent."""
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethod(
                    ...
                    description=f"Authenticate Aizen using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="aizen-agent", version=AIZEN_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Aizen on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from aizen_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            ...
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["aizen-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Aizen Agent v{AIZEN_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Aizen has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
