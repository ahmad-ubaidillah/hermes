...
# via shell expansions like $HOME or $AIZEN_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_AIZEN_ENV_PATH = (
    r'(?:~\/\.aizen/|'
    r'(?:\$home|\$\{home\})/\.aizen/|'
    r'(?:\$aizen_home|\$\{aizen_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart aizen-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart aizen-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(aizen|gateway|cli\.py)\b', "kill aizen/gateway process (self-termination)"),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["AIZEN_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "AIZEN_SPINNER_PAUSE" in os.environ:
            del os.environ["AIZEN_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    session_key = os.getenv("AIZEN_SESSION_KEY", "default")
    if is_approved(session_key, pattern_key):
        return {"approved": True, "message": None}

    is_cli = os.getenv("AIZEN_INTERACTIVE")
    is_gateway = os.getenv("AIZEN_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("AIZEN_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("AIZEN_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("AIZEN_INTERACTIVE")
    is_gateway = os.getenv("AIZEN_GATEWAY_SESSION")
    is_ask = os.getenv("AIZEN_EXEC_ASK")
