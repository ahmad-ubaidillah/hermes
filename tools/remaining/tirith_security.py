...
it is automatically downloaded from GitHub releases to $AIZEN_HOME/bin/tirith.
...
def _get_aizen_home() -> str:
    """Return the Aizen home directory, respecting AIZEN_HOME env var."""
    return str(get_aizen_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_aizen_home(), ".tirith-install-failed")
...
def _aizen_bin_dir() -> str:
    """Return $AIZEN_HOME/bin, creating it if needed."""
    d = os.path.join(_get_aizen_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $AIZEN_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_aizen_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $AIZEN_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $AIZEN_HOME/bin/tirith
    ...
    aizen_bin = os.path.join(_aizen_bin_dir(), "tirith")
    if os.path.isfile(aizen_bin) and os.access(aizen_bin, os.X_OK):
        _resolved_path = aizen_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return aizen_bin
