    ...
    async def stop(self):
        ...
        if self._proc:
            ...
            try:
                ...
                logger.warning("Failed to parse LSP response: %s", e)
            except Exception as e:
                logger.error(f"LSP client error: %s", e)
            ...
            for item in result["items"]:
                diag = LSPDiagnostic(
                    ...
                    severity=self.SEVERITY_MAP.get(
                        item.get("severity", 3), "information"
                    ),
...
class LSPClientSync:
    ...
    def get_diagnostics(self, file_path: str) -> List[LSPDiagnostic]:
        return self._loop.run_until_complete(self._client.get_diagnostics(file_path))
