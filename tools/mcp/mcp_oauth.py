...
``httpx.Auth``) with Aizen-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.aizen/mcp-tokens/
# ---------------------------------------------------------------------------
...
class AizenTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("AIZEN_HOME", Path.home() / ".aizen"))
...
def build_oauth_auth(server_name: str, server_url: str):
    ...
    storage = AizenTokenStorage(server_name)
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    AizenTokenStorage(server_name).remove()
