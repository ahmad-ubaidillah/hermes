...
discovers their tools, and registers them into the aizen-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.aizen/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            aizen_home = os.path.expanduser(
                os.getenv(
                    "AIZEN_HOME", os.path.join(os.path.expanduser("~"), ".aizen")
                )
            )
            candidates = [
                os.path.join(aizen_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from aizen-* umbrella toolsets
            for ts_name, ts in TOOLSETS.items():
                if ts_name.startswith("aizen-"):
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Aizen config file.
    ...
    ``os.environ`` (which includes ``~/.aizen/.env`` loaded at startup).
    """
    try:
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Aizen registry schema format.
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into aizen-* sets.
    ...
    Also injects all MCP tools into aizen-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into aizen-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("aizen-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    and aizen-* umbrella toolset injection.
    ...
    if registered_names:
        ...
        # Inject into aizen-* umbrella toolsets for default behavior
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("aizen-"):
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``aizen tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Aizen registry.
