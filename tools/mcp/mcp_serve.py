"""
Aizen MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Aizen-specific extra)

Usage:
    aizen mcp serve
    aizen mcp serve --verbose
...
logger = logging.getLogger("aizen.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AIZEN_HOME."""
    try:
        from core.aizen_constants import get_aizen_home
        return get_aizen_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AIZEN_HOME", Path.home() / ".aizen")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from core.aizen_constants import get_aizen_home
        directory_file = get_aizen_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AIZEN_HOME", Path.home() / ".aizen")
...
class EventBridge:
    ...
    This is the Aizen equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from core.aizen_constants import get_aizen_home
            db_file = get_aizen_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("AIZEN_HOME", Path.home() / ".aizen")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Aizen MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'aizen-agent[mcp]'"
        )

    mcp = FastMCP(
        "aizen",
        instructions=(
            "Aizen Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Aizen MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
