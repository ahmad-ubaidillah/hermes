#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Aizen Skills Hub.
...
Used by aizen_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            aizen_meta = metadata.get("aizen", {})
            if isinstance(aizen_meta, dict):
                tags = aizen_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                aizen_meta = meta_block.get("aizen", {})
                if isinstance(aizen_meta, dict):
                    tags = aizen_meta.get("tags", [])
