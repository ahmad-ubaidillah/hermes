...
import yaml
from aizen_cli.config import load_env, _ENV_VAR_NAME_RE
from tools.registry import registry

logger = logging.getLogger(__name__)


# All skills live in ~/.aizen/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if os.getenv("AIZEN_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("AIZEN_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.aizen/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.aizen/skills/mlops/axolotl/SKILL.md -> "mlops"
    """
    try:
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("AIZEN_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.aizen/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``aizen skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.aizen/skills/",
                },
                ensure_ascii=False,
            )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.aizen/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `aizen skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.aizen.* first (agentskills.io convention), fall back to top-level
        aizen_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            aizen_meta = metadata.get("aizen", {}) or {}

        tags = _parse_tags(aizen_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            aizen_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
