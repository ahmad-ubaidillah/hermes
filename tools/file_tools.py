...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        # Security: block direct reads of internal Aizen cache/index files
        # to prevent prompt injection via catalog or hub metadata files.
        import pathlib as _pathlib
        from core.aizen_constants import get_aizen_home as _get_hh
        _resolved = _pathlib.Path(path).expanduser().resolve()
        _aizen_home = _get_hh().resolve()
        _blocked_dirs = [
            _aizen_home / "skills" / ".hub" / "index-cache",
            _aizen_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Aizen cache file "
