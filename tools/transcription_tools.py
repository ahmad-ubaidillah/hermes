...
LOCAL_STT_COMMAND_ENV = "AIZEN_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "AIZEN_LOCAL_STT_LANGUAGE"
...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.aizen/config.yaml.
    ...
    try:
        import yaml
        cfg_path = get_aizen_home() / "config.yaml"
        if cfg_path.exists():
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set AIZEN_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="aizen-local-stt-") as output_dir:
