...
_SNAPSHOT_STORE = get_aizen_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    Resolution order:
      ...
      4. ~/.aizen/sandboxes/singularity (fallback)
    """
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "aizen") / "aizen-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"aizen_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "aizen-overlays"
