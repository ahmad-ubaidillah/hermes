...
Data is cached in memory (1hr TTL) and on disk (~/.aizen/models_dev_cache.json)
...
# Provider ID mapping: Aizen provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    env_val = os.environ.get("AIZEN_HOME", "")
    aizen_home = Path(env_val) if env_val else Path.home() / ".aizen"
    return aizen_home / "models_dev_cache.json"
