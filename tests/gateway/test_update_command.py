...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        runner = _make_runner()
        event = _make_event()
        monkeypatch.setenv("AIZEN_MANAGED", "homebrew")

        result = await runner._handle_update_command(event)

        assert "managed by Homebrew" in result
        assert "brew upgrade aizen-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _aizen_home to tmp_path and project_root to a dir without .git
        ...
        with patch("gateway.run._aizen_home", tmp_path):
            ...
            # project_root will be the real aizen-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_aizen_binary(self, tmp_path):
        """Returns error when aizen is not on PATH and aizen_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "aizen update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m aizen_cli.main when aizen not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        (fake_root / "gateway" / "run.py").touch()
        fake_file = str(fake_root / "gateway" / "run.py")
        aizen_home = tmp_path / "aizen"
        aizen_home.mkdir()

        mock_popen = MagicMock()
        fake_spec = MagicMock()

        with patch("gateway.run._aizen_home", aizen_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Aizen update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m aizen_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "aizen_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_aizen_bin_prefers_which(self, tmp_path):
        """_resolve_aizen_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_aizen_bin

        with patch("shutil.which", return_value="/custom/path/aizen"):
            result = _resolve_aizen_bin()

        assert result == ["/custom/path/aizen"]

    @pytest.mark.asyncio
    async def test_resolve_aizen_bin_fallback(self):
        """_resolve_aizen_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_aizen_bin
        ...
        assert result == [sys.executable, "-m", "aizen_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_aizen_bin_returns_none_when_both_fail(self):
        """_resolve_aizen_bin returns None when both strategies fail."""
        from gateway.run import _resolve_aizen_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._aizen_home", aizen_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/aizen" if x == "aizen" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = aizen_home / ".update_pending.json"
        assert pending_path.exists()
        data = json.loads(pending_path.read_text())
        assert data["platform"] == "telegram"
        assert data["chat_id"] == "99999"
        assert "timestamp" in data
        assert not (aizen_home / ".update_exit_code").exists()
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (aizen_home / ".update_pending.json").exists()
        assert not (aizen_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (aizen_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = aizen_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (aizen_home / ".update_output.txt").write_text("done")
        (aizen_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (aizen_home / ".update_pending.json").write_text(json.dumps(pending))
        (aizen_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (aizen_home / ".update_pending.json").write_text(json.dumps(pending))
        (aizen_home / ".update_output.txt").write_text("x" * 5000)
        (aizen_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (aizen_home / ".update_pending.json").write_text(json.dumps(pending))
        (aizen_home / ".update_output.txt").write_text("Traceback: boom")
        (aizen_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = aizen_home / ".update_pending.json"
        output_path = aizen_home / ".update_output.txt"
        exit_code_path = aizen_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
