...
class TestSmsToolset:
    def test_aizen_sms_toolset_exists(self):
        from tools.toolsets import get_toolset

        ts = get_toolset("aizen-sms")
        assert ts is not None
        assert "tools" in ts

    def test_aizen_sms_in_gateway_includes(self):
        from tools.toolsets import get_toolset

        gw = get_toolset("aizen-gateway")
        assert gw is not None
        assert "aizen-sms" in gw["includes"]
