...
def test_save_twilio_writes_env_and_state(tmp_path: Path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("AIZEN_HOME", str(tmp_path / ".aizen"))
    ...
    env_text = (tmp_path / ".aizen" / ".env").read_text(encoding="utf-8")
    state = json.loads((tmp_path / ".aizen" / "telephony_state.json").read_text(encoding="utf-8"))
...
def test_diagnose_includes_decision_tree_and_saved_state(tmp_path: Path, monkeypatch):
    ...
    mod._save_state(
        ...
        aizen_home / "telephony_state.json",
    )
    (aizen_home / ".env").parent.mkdir(parents=True, exist_ok=True)
    (aizen_home / ".env").write_text(
