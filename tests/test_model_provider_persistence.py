"""Tests that provider selection via `aizen model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated AIZEN_HOME with a minimal string-format config."""
    home = tmp_path / "aizen"
    home.mkdir()
    ...
    monkeypatch.delenv("AIZEN_MODEL", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from aizen_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from aizen_cli.main import _model_flow_api_key_provider
        ...
        with patch("aizen_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("aizen_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from aizen_cli.main import _model_flow_copilot
        from aizen_cli.config import load_config
        ...
        with patch(
            ...
            "aizen_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "aizen_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "aizen_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from aizen_cli.main import _model_flow_copilot_acp
        from aizen_cli.config import load_config

        with patch(
            "aizen_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "aizen_cli.auth.resolve_external_process_provider_credentials",
            return_value={
