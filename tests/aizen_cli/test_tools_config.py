"""Tests for aizen_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from aizen_cli.tools_config import (
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/aizen-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("aizen_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (aizen-cli, aizen-telegram, etc.) must NOT
    ...
    ``aizen tools``, but aizen-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "aizen-cli",  # <-- the culprit
    ...
    # aizen-cli must NOT survive — it's a platform default, not an MCP server
    assert "aizen-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_aizen_telegram():
    """Same bug for Telegram — aizen-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "aizen-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "aizen-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "aizen-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from aizen_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """aizen-gateway includes list should cover all messaging platforms."""
        from aizen_cli.tools_config import PLATFORMS
        from tools.toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["aizen-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from aizen_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from aizen_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
