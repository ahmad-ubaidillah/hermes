...
import aizen_cli.doctor as doctor
import aizen_cli.gateway as gateway_cli
from aizen_cli import doctor as doctor_mod
from aizen_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "AIZEN_HOME", aizen_home)
    monkeypatch.delenv("AIZEN_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("AIZEN_INTERACTIVE")
        raise SystemExit(0)
