...
from aizen_cli.auth import _update_config_for_provider, get_active_provider
from aizen_cli.config import load_config, save_config
from aizen_cli.setup import setup_model_provider
...
def test_nous_oauth_setup_keeps_current_model_when_syncing_disk_provider(
    ...
    monkeypatch.setattr("aizen_cli.auth._login_nous", _fake_login_nous)
    monkeypatch.setattr(
        "aizen_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "aizen_cli.auth.fetch_nous_models",
...
def test_custom_setup_preserves_provider_after_wizard_save_config(
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(input_values))
    monkeypatch.setattr("aizen_cli.setup.prompt_yes_no", lambda *a, **kw: False)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("aizen_cli.main._save_custom_provider", lambda *a, **kw: None)
    monkeypatch.setattr(
        "aizen_cli.models.probe_api_models",
