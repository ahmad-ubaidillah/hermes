...
from aizen_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_skips_migration_prompt(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "aizen_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch("aizen_cli.config.get_missing_config_fields", return_value=[]), patch(
            "aizen_cli.config.check_config_version", return_value=(1, 2)
        ), patch("aizen_cli.main.sys") as mock_sys:
