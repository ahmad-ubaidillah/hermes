...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("aizen_cli.setup.ensure_aizen_home"),
            patch("aizen_cli.setup.load_config", return_value={}),
            patch("aizen_cli.setup.get_aizen_home", return_value="/tmp/.aizen"),
            patch("aizen_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "aizen config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `aizen` should not prompt for input when no provider exists and stdin is headless."""
        from aizen_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("aizen_cli.main._has_any_provider_configured", return_value=False),
            patch("aizen_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
