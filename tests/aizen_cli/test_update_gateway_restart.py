...
Ensures ``aizen update`` correctly detects running gateways managed by
...
import aizen_cli.gateway as gateway_cli
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>AIZEN_HOME</key>" in plist
...
class TestLaunchdPlistRefresh:
    ...
    def test_refresh_rewrites_stale_plist(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.aizen.gateway.plist"
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: aizen gateway run'."""
        ...
        assert "Restart it with: aizen gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart it with: aizen gateway run" in captured
