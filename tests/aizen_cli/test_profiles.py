"""Comprehensive tests for aizen_cli.profiles module.
...
from aizen_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_aizen_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and AIZEN_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.aizen/profiles)
    * AIZEN_HOME  -> tmp_path/.aizen  (so get_aizen_home() agrees)
    * Creates the bare-minimum ~/.aizen directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".aizen"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("AIZEN_HOME", str(default_home))
    return tmp_path
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("aizen_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".aizen" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_aizen_home_returns_default(self, profile_env):
        # AIZEN_HOME points to tmp_path/.aizen which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".aizen" / "profiles" / "coder"
        monkeypatch.setenv("AIZEN_HOME", str(profile_dir))
    ...
    def test_custom_path_returns_custom(self, profile_env, monkeypatch):
        ...
        monkeypatch.setenv("AIZEN_HOME", str(custom))
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("aizen")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".aizen" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("aizen_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".aizen" / "profiles" / "newname"
...
class TestCompletion:
    ...
    def test_bash_completion_has_aizen_profiles_function(self):
        script = generate_bash_completion()
        assert "_aizen_profiles" in script

    def test_zsh_completion_has_aizen_function(self):
        script = generate_zsh_completion()
        assert "_aizen" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultAizenHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_aizen_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".aizen" / "profiles"

    def test_default_aizen_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_aizen_home()
        assert home == tmp_path / ".aizen"
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".aizen" / "profiles" / "mybot"
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from aizen_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".aizen"
