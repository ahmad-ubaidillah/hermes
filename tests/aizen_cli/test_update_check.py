"""Tests for the update check mechanism in aizen_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from aizen_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    with patch("aizen_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("aizen_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("aizen_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("aizen_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    import aizen_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "aizen_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    """Dev install: falls back to Path(__file__).parent.parent when AIZEN_HOME has no git repo."""
    import aizen_cli.banner as banner
    ...
    # Point AIZEN_HOME at a temp dir with no aizen-agent/.git
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        with patch("aizen_cli.banner.os.getenv", return_value=td):
            with patch("aizen_cli.banner.subprocess.run") as mock_run:
