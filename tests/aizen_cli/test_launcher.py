"""Tests for the top-level `./aizen` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./aizen` should use `aizen_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "aizen"
    called = []

    fake_main_module = types.ModuleType("aizen_cli.main")

    def fake_main():
        called.append("aizen_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "aizen_cli.main", fake_main_module)
