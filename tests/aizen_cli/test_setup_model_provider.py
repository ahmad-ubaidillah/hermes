...
from aizen_cli.config import load_config, save_config, save_env_value
from aizen_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aizen_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("aizen_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("aizen_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("aizen_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
...
def test_setup_keep_current_config_provider_uses_provider_specific_model_menu(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aizen_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("aizen_cli.models.provider_model_ids", lambda provider: [])
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_copilot_uses_gh_auth_and_saves_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aizen_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("aizen_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("aizen_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("aizen_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("aizen_cli.auth.get_auth_status", fake_get_auth_status)
    monkeypatch.setattr(
        "aizen_cli.auth.resolve_api_key_provider_credentials",
        lambda provider_id: {
...
def test_setup_copilot_acp_uses_model_picker_and_saves_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aizen_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("aizen_cli.auth.get_auth_status", lambda provider_id: {"logged_in": provider_id == "copilot-acp"})
    monkeypatch.setattr(
        "aizen_cli.auth.resolve_api_key_provider_credentials",
        lambda provider_id: {
            "provider": "copilot",
...
def test_setup_switch_custom_to_codex_clears_custom_endpoint_and_updates_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aizen_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("aizen_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("aizen_cli.auth._login_openai_codex", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "aizen_cli.auth.resolve_codex_runtime_credentials",
        lambda *args, **kwargs: {
...
def test_setup_summary_marks_codex_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'aizen setup' to configure" not in output
