"""Tests for aizen_cli configuration management."""
...
class TestGetAizenHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AIZEN_HOME", None)
            home = get_aizen_home()
            assert home == Path.home() / ".aizen"

    def test_env_override(self):
        with patch.dict(os.environ, {"AIZEN_HOME": "/custom/path"}):
            home = get_aizen_home()
            assert home == Path("/custom/path")


class TestEnsureAizenHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"AIZEN_HOME": str(tmp_path)}, clear=False):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from aizen_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
