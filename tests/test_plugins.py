"""Tests for the Aizen plugin system (aizen_cli.plugins)."""
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.aizen/plugins/ are discovered."""
        plugins_dir = tmp_path / "aizen_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("AIZEN_HOME", str(tmp_path / "aizen_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AIZEN_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".aizen" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under aizen_plugins.<name>."""
        plugins_dir = tmp_path / "aizen_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("AIZEN_HOME", str(tmp_path / "aizen_test"))

        # Clean up any prior namespace module
        sys.modules.pop("aizen_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "aizen_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="aizen_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import aizen_cli.plugins as plugins_mod
...
# in PluginContext (aizen_cli/plugins.py).  The tests referenced _plugin_commands,
