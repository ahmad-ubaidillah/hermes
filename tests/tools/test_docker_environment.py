...
def test_execute_uses_aizen_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_aizen_env_vars", lambda: {"GITHUB_TOKEN": "value_from_dotenv"})
...
def test_execute_prefers_shell_env_over_aizen_dotenv(monkeypatch):
    env = _make_execute_only_env(["GITHUB_TOKEN"])
    popen_calls = []
