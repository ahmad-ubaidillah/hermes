...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        aizen_home.mkdir()
        (aizen_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(aizen_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.aizen/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        aizen_home = tmp_path / ".aizen"
        aizen_home.mkdir()
        (aizen_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        aizen_home = tmp_path / ".aizen"
        aizen_home.mkdir()
        (aizen_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        aizen_home = tmp_path / ".aizen"
        aizen_home.mkdir()
        (aizen_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mount["container_path"] == "/root/.aizen/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        aizen_home = tmp_path / ".aizen"
        (aizen_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"AIZEN_HOME": str(aizen_home)}):
            mount = get_skills_directory_mount(container_base="/home/user/.aizen")

        assert mount["container_path"] == "/home/user/.aizen/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.aizen/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.aizen/skills/cat/myskill/scripts/run.sh" in paths
