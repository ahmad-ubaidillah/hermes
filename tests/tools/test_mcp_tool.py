...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("aizen_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("aizen_cli.config.load_config", return_value={"mcp_servers": servers}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("aizen_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_aizen_schema(self):
...
class TestDiscoverAndRegister:
    ...
    def test_toolset_created(self):
        ...
        with patch("tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("tools.toolsets.create_custom_toolset", mock_create):
            asyncio.run(
...
class TestToolsetInjection:
    def test_mcp_tools_added_to_all_aizen_toolsets(self):
        """Discovered MCP tools are dynamically injected into all aizen-* toolsets."""
        from tools.mcp_tool import MCPServerTask
        ...
        fake_toolsets = {
            "aizen-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
            "aizen-telegram": {"tools": ["terminal"], "description": "TG", "includes": []},
            "aizen-gateway": {"tools": [], "description": "GW", "includes": []},
            "non-aizen": {"tools": [], "description": "other", "includes": []},
        }
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), \
             ...
             patch("tools.toolsets.TOOLSETS", fake_toolsets):
            from tools.mcp_tool import discover_mcp_tools
            result = discover_mcp_tools()

        assert "mcp_fs_list_files" in result
        # All aizen-* toolsets get injection
        assert "mcp_fs_list_files" in fake_toolsets["aizen-cli"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["aizen-telegram"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["aizen-gateway"]["tools"]
        # Non-aizen toolset should NOT get injection
        assert "mcp_fs_list_files" not in fake_toolsets["non-aizen"]["tools"]
        # Original tools preserved
        assert "terminal" in fake_toolsets["aizen-cli"]["tools"]
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "aizen-cli": {"tools": [], "description": "CLI", "includes": []},
        }
...
class TestMCPSelectiveToolLoading:
    ...
    def _run_discover(self, name, tool_names, config, session=None):
        ...
        async def run():
            with patch("tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch("tools.registry.registry", mock_registry), \
                 patch("tools.toolsets.create_custom_toolset"):
