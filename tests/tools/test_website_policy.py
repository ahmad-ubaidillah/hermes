...
def test_check_website_access_uses_dynamic_aizen_home(monkeypatch, tmp_path):
    aizen_home = tmp_path / "aizen-home"
    aizen_home.mkdir()
    ...
    # Invalidate the module-level cache so the new AIZEN_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old AIZEN_HOME set by the autouse _isolate_aizen_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing AIZEN_HOME to tmp_path
