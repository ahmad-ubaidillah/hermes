...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aizen_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_aizen_bin(self):
        """After _INSTALL_FAILED, manual install in AIZEN_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        aizen_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(aizen_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(aizen_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aizen_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == aizen_bin
            assert _tirith_mod._resolved_path == aizen_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# AIZEN_HOME isolation
# ---------------------------------------------------------------------------

class TestAizenHomeIsolation:
    def test_aizen_bin_dir_respects_aizen_home(self):
        """_aizen_bin_dir must use AIZEN_HOME, not hardcoded ~/.aizen."""
        from tools.tirith_security import _aizen_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AIZEN_HOME": tmpdir}):
            result = _aizen_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_aizen_home(self):
        """_failure_marker_path must use AIZEN_HOME, not hardcoded ~/.aizen."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AIZEN_HOME": "/custom/aizen"}):
            result = _failure_marker_path()
        assert result == "/custom/aizen/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AIZEN_HOME; verify it's active."""
        aizen_home = os.getenv("AIZEN_HOME")
        assert aizen_home is not None, "AIZEN_HOME should be set by conftest"
        assert "aizen_test" in aizen_home, "Should point to test temp dir"

    def test_get_aizen_home_fallback(self):
        """Without AIZEN_HOME set, falls back to ~/.aizen."""
        from tools.tirith_security import _get_aizen_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AIZEN_HOME entirely
            os.environ.pop("AIZEN_HOME", None)
            result = _get_aizen_home()
        assert result == os.path.join(os.path.expanduser("~"), ".aizen")
