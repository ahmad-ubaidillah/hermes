"""
Tests for timezone support (aizen_time module + integration points).
...
import core.aizen_time as aizen_time


# =========================================================================
# aizen_time.now() — core helper
# =========================================================================

class TestAizenTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        aizen_time.reset_cache()

    def teardown_method(self):
        aizen_time.reset_cache()
        os.environ.pop("AIZEN_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["AIZEN_TIMEZONE"] = "Asia/Kolkata"
        result = aizen_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["AIZEN_TIMEZONE"] = "UTC"
        result = aizen_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["AIZEN_TIMEZONE"] = "America/New_York"
        result = aizen_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["AIZEN_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="aizen_time"):
            result = aizen_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["AIZEN_TIMEZONE"] = "Europe/London"
        tz = aizen_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["AIZEN_TIMEZONE"] = "Not/A/Timezone"
        tz = aizen_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["AIZEN_TIMEZONE"] = "Asia/Tokyo"
        assert aizen_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When AIZEN_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["AIZEN_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When AIZEN_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("AIZEN_TIMEZONE", None)
    ...
    def test_aizen_timezone_not_leaked_to_child(self):
        """AIZEN_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["AIZEN_TIMEZONE"] = "Asia/Kolkata"

        with patch("tools.model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("AIZEN_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=aizen_tz) which shifted
        absolute time when system-local tz != Aizen tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_aizen_tz(self):
        """Already-aware datetimes should be normalized to Aizen tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Aizen tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Aizen tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Aizen is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Aizen tz
        ...
        # Use a Aizen timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _aizen_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["AIZEN_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        aizen_time.reset_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Aizen timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["AIZEN_TIMEZONE"] = "US/Eastern"
        aizen_time.reset_cache()
