...
version: 1.0.0
author: arceus77-7, enhanced by Aizen Agent
license: MIT
metadata:
  aizen:
...
- `tmux` available for stable authenticated sessions during Aizen terminal calls (desktop app flow only)
...
### Service Account (recommended for Aizen)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.aizen/.env` (the skill will prompt for this on first load).
...
## Aizen Execution Pattern (desktop app flow)

Aizen terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/aizen-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/aizen-op.sock"
