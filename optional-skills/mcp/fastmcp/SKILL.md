...
- smoke-test a server with the FastMCP CLI before wiring it into Aizen or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Aizen. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.aizen/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.aizen/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Aizen integration, either:

- configure the server in `~/.aizen/config.yaml` using the `native-mcp` skill, or
...
### Aizen cannot see the deployed server

The server-building part may be correct while the Aizen config is not. Load the `native-mcp` skill and configure the server in `~/.aizen/config.yaml`, then restart Aizen.
